...
from suite.kinetix.environment.env_state import EnvParams, EnvState, StaticEnvParams
from suite.kinetix.environment.ued.ued import make_reset_fn_from_config
from suite.kinetix.environment.utils import (
    ActionType,
    ContinuousActions,
    DiscreteActions,
    EntityObservations,
...
def make_kinetix_env(
    ...
    print("action_type:", action_type, "observation_type:", observation_type)
    print("action_type:", action_type.name, "observation_type:", observation_type.name)

    if action_type.name == "DISCRETE":
        action_type_cls = DiscreteActions
    elif action_type.name == "CONTINUOUS":
        action_type_cls = ContinuousActions
    elif action_type.name == "MULTI_DISCRETE":
    ...
    # observations
    if observation_type.name == "PIXELS":
        obs_type_cls = PixelObservations
    elif observation_type.name == "SYMBOLIC_FLAT":
        obs_type_cls = SymbolicObservations
    elif observation_type.name == "SYMBOLIC_ENTITY":
        obs_type_cls = EntityObservations
    elif observation_type.name == "SYMBOLIC_FLAT_PADDED":
