...
class IPHYRE():
    ...
    def add_joint(self):
        bodies_list = list(self.space.bodies)
        for (b1, b2) in PARAS[self.game]['joint']:
            c: pymunk.Constraint = pymunk.PinJoint(bodies_list[b1], bodies_list[b2])
            self.space.add(c)

    def add_spring(self):
        bodies_list = list(self.space.bodies)
        for (b1, b2) in PARAS[self.game]['spring']:
            c = pymunk.DampedSpring(bodies_list[b1], bodies_list[b2], (0, 0), (0, 0), 20, 1, 0.3)
            self.space.add(c)
    ...
    def eliminate(self, p):
        bodies_list = list(self.space.bodies)
        for i, body in enumerate(bodies_list[:-self.num_ball]):
    ...
    def examine_success(self):
        success = 0
        bodies_list = list(self.space.bodies)
        for ball in bodies_list[-self.num_ball:]:
    ...
    def init_screen(self):
        ...
        self.index_font = pygame.font.Font(None, 60)
    ...
    def reset(self, use_images=False):
        ...
        for body in bodies_list:
            shape = list(body.shapes)[0]
            self.space.remove(shape, shape.body)
        constraints_list = list(self.space.constraints)
        for joint in constraints_list:
    ...
    # ==================== 新增代码 (2/3) ====================
    # 新增一个私有方法，用于在屏幕上绘制可消除方块的索引
    def _draw_action_indices(self):

        """
        Draws the index number on each eliminable block.
        The index corresponds to the order in action_space.
        """
        if not self.screen or not hasattr(self, 'index_font'):
            return  # Can't draw if screen/font isn't initialized

        all_property = self.get_all_property()
        action_index = 0
        for prop in all_property:
            # prop[-4] is the 'eli' flag. We only care about eliminable blocks.
            # np.any(prop[:4]) is a check to ensure it's not a padding row (all zeros).
            if prop[-4] == 1 and np.any(prop[:4]):
                # Calculate the center of the block
                x = (prop[0] + prop[2]) / 2
                y = (prop[1] + prop[3]) / 2

                text_surface = self.index_font.render(str(action_index), True, pygame.Color("blue"))

                # Get the rectangle of the text surface and center it on the block's coordinates
                text_rect = text_surface.get_rect(center=(x, y))

                # Draw the text onto the main screen
    ...
    def collect_initial_data(self, save_path='./game_initial_data/'):
        ...
        pygame.display.flip()
        pygame.image.save(self.screen, game_path + f'{self.game}.png')
        ...
        for i, act_list in enumerate(act_lists):  # the step number of each action can be variant
            ...
            while time_count < self.max_time:
                ...
                if interval_cal == interval or interval_cal == 0:
                    ...
                    pygame.display.flip()
                    pygame.image.save(self.screen, img_path + f'{save_count:03d}.png')
                    save_count += 1
                    if save_count == int(15 * self.FPS): break
